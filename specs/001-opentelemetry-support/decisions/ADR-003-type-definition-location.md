...
- Uses string literals for ergonomic API: `type: "http"` instead of `kind: InstrumentKinds.HTTP`
...
const id = Bun.telemetry.attach({
  kind: "http", // ← Ergonomic, self-documenting
...
The Bun runtime uses numeric enum values internally for performance and type-safe routing. This is an implementation detail not exposed to application code. String-to-numeric conversion happens in the Zig bridge layer during `attach()`, ensuring compile-time validation while keeping the public API clean.
...
- Self-documenting: `kind: "http"` is clear and intuitive
- No import needed: strings are primitive values
- More stable: string values won't change
- Familiar: matches common JavaScript patterns (event types, etc.)

### 3. Internal Performance

The runtime uses numeric enums internally for efficiency:

- Compile-time validation of Zig enum mapping
- O(1) dispatch via array indexing
- Clear documentation of all possible values in source code
- String validation happens once during `attach()`, not on every operation
...
### Option 4: Expose Numeric Enums in Public API
...
1. **String Validation**: String literals validated at runtime during `attach()`
   - _Mitigation_: Validation happens once per instrument, comprehensive error messages
2. **Module Augmentation**: bun-otel extends Bun namespace via `declare module "bun"`
   - _Mitigation_: Standard TypeScript pattern, well-documented
3. **Documentation Split**: Users must know where to find types
   - _Mitigation_: Clear guidelines in specs, ADRs document rationale
